    ...
    */
    ...
    Uploader.prototype = {
        ...
        remove: function (taskId) {
            ...
            //this.list.splice(task.indexPage, 1);
        ...
        start: function () {
            ...
            var _isStart = count <= 0 || index >= count || workerIdle <= 0;
            if (_isStart) return self;
        ...
        upload: function (task) {
            ...
            var _taskOk =!task || task.state != UPLOAD_STATE_READY || task.skip;

            if (_taskOk) return self.start();

            task.url = self.url;
            self.workerIdle--;

            self.fire("uploadOne", task, function (result) {
